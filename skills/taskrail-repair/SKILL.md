...
1. **Inspect.** Run `taskrail validate --json` and read the violations. Repair
...
2. **Dry-run.** Run `taskrail repair`. This defaults to a dry run: it prints the
...
4. **Apply.** Run `taskrail repair --apply` to write STATE.md and re-run
   validation.
5. **Re-validate.** Run `taskrail validate` and confirm the state is valid. Any
...
- never hand-edit `planning/STATE.md` frontmatter or task status fields; route
  every correction through `taskrail repair`
