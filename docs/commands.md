...
| Mode-dependent initialization | `init` | Fresh, unmarked-standard, and current-layout adoption/repair paths may write immediately; detected migration or retrofit paths preview unless `--apply` is supplied. Fresh/adopted writes, including `--with-skills`, publish as one locked normal transaction. Layout-creating outcomes also append a marked `.gitignore` block that keeps `planning/artifacts/` output out of Git status, preserving existing user rules; marker-only adoption writes nothing else. A repository at layout 1 reports the read-only layout 2 upgrade preview instead — that upgrade publishes the same `.gitignore` candidate inside its one durable migration transaction (see [migration.md](migration.md)). |
