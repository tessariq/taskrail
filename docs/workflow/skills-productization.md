...
| `taskrail-spec-review` | Shippable | Stages four independent advisory post-spec lens observations per round and publishes one human-dispositioned digest-bound history bundle whose manifest declares its round history and labels dispositions as unverified caller-recorded claims; it performs no semantic writes (T-162, T-407). |
...
| `taskrail-sdd-handoff` | Shippable | Converts no source bytes: it proposes a conservative, content-based handoff from an operator-selected reviewed OpenSpec or Spec Kit artifact set into existing Taskrail spec, import, and decomposition flows, stopping on ambiguity (T-202). |
