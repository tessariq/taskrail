...
func (c Capability) Includes(other Capability) bool {
	...
	if len(outer.Writes) > 0 && (len(inner.Writes) == 0 || !writesSubset(inner.Writes, outer.Writes)) {
		return false
	}
...
// writesSubset permits an owner to grant a task-scoped directory ending in `/`.
// A verification artifact name is generated by the child, so its parent cannot
// enumerate the final timestamp and verification ID before delegation.
func writesSubset(inner, outer []string) bool {
	for _, candidate := range inner {
		allowed := false
		for _, grant := range outer {
			if candidate == grant || strings.HasSuffix(grant, "/") && strings.HasPrefix(candidate, grant) {
				allowed = true
				break
			}
		}
...
func (c Capability) AllowsWrites(paths []string) error {
	...
	for _, path := range paths {
		if !writeAllowed(strings.TrimSpace(path), bound) {
...
func writeAllowed(path string, bound []string) bool {
	for _, grant := range bound {
		if path == grant || strings.HasSuffix(grant, "/") && strings.HasPrefix(path, grant) {
			return true
		}
	}
	return false
