...
func TestMachineInventoryClassifiesConstructedCommands(t *testing.T) {
	...
	for _, row := range []string{"`validate`", "`status`", "`task new`", "`task loop allow`", "`task loop hold`", "`task loop clear`", "`task loop list`", "`spec activate`", "`local status`", "`local path`", "`prompt list`", "`prompt show`", "`prompt render`", "`review publish`"} {
		if origins[row] != MachineOriginConstructed {
			t.Errorf("%s is built by the current CLI, inventory says %q", row, origins[row])
		}
	}
	for _, row := range []string{"`task author`"} {
