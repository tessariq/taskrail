...
func TestMachineInventoryClassifiesConstructedCommands(t *testing.T) {
	...
	for _, row := range []string{"`validate`", "`status`", "`task new`", "`spec activate`", "`local status`", "`local path`", "`prompt list`", "`prompt show`", "`review publish`"} {
		if origins[row] != MachineOriginConstructed {
			t.Errorf("%s is built by the current CLI, inventory says %q", row, origins[row])
		}
	}
	for _, row := range []string{"`task loop list`", "`prompt render`", "`loop` dry-run"} {
