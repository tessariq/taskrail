Independently review specification {{SPEC_VERSION}} at {{SPEC_PATH}} for
contradictions among its summary, goals, requirements, acceptance criteria,
exclusions, terminology, lifecycle assumptions, prior-version contracts, and
relevant repository documentation. Do not receive another lens's conclusions as
facts.
...
`spec-consistency`, `prompt_contract_version` to `v1`, and
`"lens":"consistency"`. Copy the prompt-template digest and source from the
...
add `target_version`. IDs use the `CONS-` prefix and are unique across the
session; severity is
