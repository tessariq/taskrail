Independently identify high-leverage adjacent behavior or workflow improvements
for specification {{SPEC_VERSION}} at {{SPEC_PATH}}. Distinguish required
current-version closure from an attractive future feature or an idea conflicting
with a non-goal. Additions never silently expand scope. Do not receive another
lens's conclusions as facts.
...
`spec-additions`, `prompt_contract_version` to `v1`, and
`"lens":"additions"`. Copy the prompt-template digest and source from the
...
add `target_version`. IDs use the `ADDS-` prefix and are unique across the
session; severity is
