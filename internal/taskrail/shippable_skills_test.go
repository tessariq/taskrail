...
func TestShippableSkillsConsumeStructuredResultsAsJSON(t *testing.T) {
	required := map[string][]string{
		...
		"taskrail-spec-review":          {"} spec show <version> --json", "} prompt render spec-consistency --spec <version> --review <proposal-dir>/round-1-consistency.json --json", "} review publish --type spec --proposal <proposal-dir> --destination <planning-dir>/reviews/spec/<version>/<session-id> --spec <version> --expect-spec-sha256 <digest> --json"},
...
func TestDecomposeSkillDefinesReviewedV2Flow(t *testing.T) {
	assertSkillReferences(t, "taskrail-decompose",
		"ImportDraft v2",
		"schema-2 post-spec review bundle",
		"Schema-1 post-spec bundles remain readable historical records",
...
func TestSpecReviewSkillStagesIndependentDigestBoundLenses(t *testing.T) {
	assertSkillReferences(t, "taskrail-spec-review",
		...
		"round-1-consistency.json",
		"declared round history",
		"unverified-caller-recorded-claims",
		"must stop rather than manufacture them",
		"never author dispositions, approval times, or identity claims",
		"exactly one disposition",
		"`round`, `finding_id`, `lens`, `severity`, `disposition`, `rationale`",
		"must equal the referenced finding",
		"`resulting_spec_ref` is required for accepted",
		"`target_version` is required for deferred",
		"a high or medium finding cannot be deferred",
		...
		"never erase earlier findings",
		"repeats_earlier_spec",
	)
}
