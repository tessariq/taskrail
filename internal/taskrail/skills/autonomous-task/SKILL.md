...
1. Run `${TASKRAIL:-taskrail} validate --json`, then use `${TASKRAIL:-taskrail} task show <task-id> --json`
   and `${TASKRAIL:-taskrail} spec show <version> --json` for managed task and
   specification bytes. Do not open logical managed paths directly.
...
- do not hand-edit machine-managed state or task statuses
