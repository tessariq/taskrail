...
func renderImportPrompt(source, target, markdown string) string {
	...
	b.WriteString("This is the compatibility ImportDraft v1 contract.\n")
	...
	b.WriteString("  - `body` (string): legacy input retained by schema v1 but ignored on apply.\n")
	...
	b.WriteString("- Give each task one independently meaningful outcome and observable result.\n")
	b.WriteString("- Split independently useful outcomes when they have separate acceptance or durable\n")
	b.WriteString("  oracles, dependencies, or operator gates. Merge fragments whose code, tests,\n")
	b.WriteString("  documentation, migration, and integration establish only one result.\n")
	b.WriteString("- Do not split one outcome by file, layer, discipline, phase, or estimate.\n")
	b.WriteString("- Name the task that owns required integrated behavior and wire real dependencies.\n")
	b.WriteString("- Only reference a `spec_ref` heading that exists; apply verifies it.\n\n")
	b.WriteString("Tasks must omit `loop_policy` and `loop_reason`; they are not v1 members and imported\n")
	b.WriteString("tasks remain implicitly held. Apply emits the standard non-empty Description,\n")
	b.WriteString("Acceptance, and Verification Notes scaffold plus Implementation Notes. The supplied\n")
	b.WriteString("body is legacy input and is ignored on apply. This structural scaffold does not\n")
	b.WriteString("certify that an imported task is semantically right-sized; review the outcome,\n")
	b.WriteString("boundaries, evidence, dependencies, and integration owner before execution.\n\n")
