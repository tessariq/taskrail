...
- **Promote personal planning deliberately** — `local promote` previews the exact local semantic files that would become committed planning. `local promote --apply` publishes that set and the committed storage marker through one recoverable transaction, then leaves it visible for normal Git review and staging without making a Git commit. `--with-skills` is the separate explicit consent to make unchanged managed skill copies visible by removing only their exact exclusions; it never copies, refreshes, or commits skill bytes.
