...
| `taskrail coverage` | Report advisory spec coverage for the active spec in two dimensions — decomposition (gating) and report-only implementation — plus orphan tasks and drift. Read-only; never fails `validate`. `--json`. |
