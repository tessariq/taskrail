...
| `taskrail coverage` | Report advisory spec coverage for the active spec in two dimensions — decomposition (gating) and report-only implementation — plus orphan tasks and drift. Read-only; never fails `validate`. `--min <pct>` opts into CI gating: exits non-zero when decomposition coverage is below the threshold (report unchanged; `N/A` never gates). `--json`. |
