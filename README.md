...
- **Draft missing work** — the optional `taskrail-decompose` skill authors an ImportDraft v2, runs at most two fresh-context adversarial passes, publishes the digest-bound bundle, then applies those exact reviewed bodies; `taskrail-gap` turns structural gap signals into reviewable proposals. Only an explicit `task new` or `import --apply` writes tracked tasks, and imported tasks remain implicitly held.
