...
- **Inspect workflow prompts** — `prompt list` reports the versioned embedded catalog and committed replacement source, while `prompt show <id>` prints the resolved template or `--builtin` bytes without creating prompt files.
- **Publish task review evidence** — `review publish --type task` validates one ignored proposal against exact task and spec snapshots, then copies its `review.json` byte-for-byte into one absent durable session directory. Use `--dry-run` to inspect the same candidate without writing it.
...
|---|---|---|
| Read-only | `validate`, `status`, `stats`, `coverage`, `task show`, `spec list/show/diff`, `prompt list/show`, `lock status` | Inspect only; never rewrite tracked planning state. |
...
| Review publisher | `review publish --type task` | Validates an ignored proposal and exact reviewed subjects; `--dry-run` is read-only, while apply takes the writer lock and creates one absent review session directory without changing task lifecycle or planning state. |
