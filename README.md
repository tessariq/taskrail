...
- **Promote personal planning deliberately** — `local promote` previews the exact local semantic files that would become committed planning. `local promote --apply` publishes that set and the committed storage marker through one recoverable transaction, then leaves it visible for normal Git review and staging without making a Git commit. Artifacts, runtime data, and installed packaged skills remain local; skill visibility requires its separate explicit flow.
- **See where work stands** — `status`, `stats`, and `coverage` report a live snapshot, aggregate metrics, and advisory spec-linkage, all read-only. `stats --active-spec` narrows task-derived metrics and dependency graphs to active-spec subjects while retaining their off-spec dependency context. `status` also breaks down open work (`todo`/`in_progress`/`blocked`) by how much targets the active spec versus points away from it, listing the away tasks and their `spec_ref`; the away set matches the active-spec filter `next` uses for idle selection.
...
- **Publish review evidence** — `review publish --type task` validates one ignored task proposal against exact task and spec snapshots; `--type spec` validates an approved ignored four-lens bundle and exact selected spec snapshot; `--type decomposition` validates a complete reviewed draft/trace bundle against exact selected-spec and published spec-review snapshots; and `--type workflow` atomically publishes one Git-bound immutable report with its Taskrail-derived workflow memory index. Use `--dry-run` to inspect the same candidate without writing it.
- **Draft and review work** — the optional `taskrail-decompose` skill authors an ImportDraft v2, runs at most two fresh-context adversarial passes, publishes the digest-bound bundle, then applies those exact reviewed bodies; `taskrail-sdd-handoff` conservatively routes already reviewed OpenSpec or Spec Kit artifacts into those existing Taskrail flows without applying or synchronizing them; `taskrail-gap` turns structural gap signals into reviewable proposals; and `taskrail-workflow-adversarial` runs bounded post-implementation probes only in an isolated sandbox before publishing report/memory evidence. Only an explicit `task new` or `import --apply` writes tracked tasks, and workflow review never promotes findings.
- **Handle the messy parts** — `task show <id>` reads one exact task's persisted Markdown through the active storage context. `block`/`unblock` park and resume work; `task release <id> --reason "..."` deliberately returns interrupted active work to `todo` without fabricating blocker or cancellation history; `task new` scaffolds a task, `task rename` re-slugs it, `task repoint` moves its `spec_ref`, `task author` applies one digest-bound reviewed body, and `task dependency add|remove` changes one reviewed dependency edge. When a crashed writer leaves the repository mutation lock behind, `lock status` inspects it read-only and `lock clear` removes exactly the observed stale lock — never automatically, and never while its owner is provably alive on this host. When a crashed durable transaction leaves a recovery fence behind, `recover <transaction-id>` previews and — with `--apply` — performs the one mechanically safe action (restore originals, accept the validated candidate, or clear the fence).
...
| Local semantic promotion | `local promote` | Previews ignored local specs, planning, prompts, exclusions, and installed-skill preservation with zero writes. `--apply` atomically publishes semantic bytes into committed paths, removes only their exact local sources and local-mode exclusions, and never creates a Git commit. Artifacts, runtime data, and installed skills remain private. |
...
mutation writers — `task new`, `task rename`, `task repoint`, `task author`, and
