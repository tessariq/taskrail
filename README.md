...
- **Publish review evidence** — `review publish --type task` validates one ignored task proposal against exact task and spec snapshots; `review publish --type spec` validates an approved ignored four-lens bundle and exact selected spec snapshot. Both publish unchanged files into one absent durable session directory, and `--dry-run` inspects the same candidate without writing it.
...
| Review publisher | `review publish --type task\|spec` | Validates an ignored proposal and exact reviewed subjects; `--dry-run` is read-only, while apply takes the writer lock and creates one absent review session directory without changing task lifecycle or planning state. |
