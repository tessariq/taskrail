...
assert_review_prompt() {
  ...
  assert_contains "$name correctness delegation" "$value" "Then run a correctness-review round"
  assert_contains "$name review round ceiling" "$value" "Run at most two correctness-review rounds"
  assert_contains "$name reviewers per round" "$value" "one to three fresh review subagents"
  assert_contains "$name distinct review lenses" "$value" "a different explicit review lens"
  ...
  assert_contains "$name post-fix simplification" "$value" "After fixing any findings from a round, use a fresh code-simplifier subagent"
  assert_contains "$name second round terminal" "$value" "After the second round, do not request another correctness review"
  assert_contains "$name terminal green checks" "$value" "final applicable build and test checks pass"
  assert_contains "$name terminal unreviewed bytes" "$value" "The resulting simplified bytes do not require a third review"
  assert_contains "$name no budget follow-up" "$value" "review-round limit never turns current work into a follow-up"
  assert_contains "$name terminal outcome set" "$value" "parent accepts only completed/pass or blocked/fail"
