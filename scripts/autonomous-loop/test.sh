...
assert_review_prompt() {
  ...
  assert_contains "$name review round ceiling" "$value" "Run at most two broad correctness-review rounds"
  ...
  assert_contains "$name final diff finding" "$value" "resulting bytes are unreviewed and must use the rework path"
