...
- A3. Ambiguous approval, ownership, requirement meaning, semantic sizing under
  the T-251 rubric, integration ownership, dependencies, or target anchors causes
  the workflow to stop for operator review. The skill does not guess task
  boundaries, silently omit uncertain material, or present ambiguity as a
  completed handoff.
...
- A3: Negative behavioral cases should include an artifact whose filename suggests
  approval but whose content does not, conflicting requirements, ambiguous
  split/merge or integration ownership, an unresolved target anchor, and incomplete
  task evidence; each must stop with an explicit operator decision rather than
