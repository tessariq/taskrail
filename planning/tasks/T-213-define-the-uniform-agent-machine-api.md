...
- Status reports exact storage mode/root so skills select delivery behavior without
  reading the layout marker or deriving local paths; roots are `.` for committed
  and `.taskrail/local` for local.
...
- Map every JSON-capable command to exact success/refusal goldens, stdout/stderr checks, human/JSON exit parity, storage-aware status/init fixtures, mixed-class snapshot fixtures, and unknown-field decoder tests.
