...
- A2. The generic trust contract requires one non-bare Git worktree whose root equals the configured Taskrail root. Every consumed set is classified as clean tracked Git input or a complete private managed-overlay snapshot and is reached without link, junction, mount-substitution, or reparse traversal; unsupported or mixed identity guarantees fail closed.
- A3. Shared cleanliness and text contracts distinguish every committed and local refusal class, including staged, unstaged, conflicted, sparse, external-untracked, mixed-storage, unavailable, aliased, special, filter-indeterminate, BOM, NUL, invalid UTF-8, oversized, and racily changed inputs rather than choosing one convenient byte representation.
- A4. The profile-neutral descriptor contract has exactly `profile{name,version}`, `trust`, `root`, `aggregate_sha256`, and non-null `sources`; each source has exactly `role`, `path`, `size`, and `sha256`. Names, versions, trust, roles, paths, sizes, digest spelling, duplicate rejection, nullability, and deterministic ordering follow v0.7's stated boundaries.
...
- A7. Source command machine output extends the inherited common envelope. Warning, error-details, and violation objects have the exact fields and nullability from v0.7; stable error codes, unsigned-byte path sorting, phase-stable violation order, one-document JSON output, and matching human/JSON exit classification are contractually enforced.
- A8. Inspect result contracts expose exactly the profile, trust, root, aggregate digest, ordered sources, and nullable matching-receipt summary. Import result contracts expose exact trust-labelled source, spec, draft, and mapping identities plus `applied`, receipt ID/path, and ordered predicted/resulting tasks under envelope generation 3. Additional internal claims cannot leak into either public shape.
...
- A1-A3: Unit and repository-boundary integration matrices should cover every accepted path form and each committed/local filesystem refusal class, including unsupported no-follow, mixed trust, external untracked, and incomplete managed-overlay checks. Assert deterministic error classification and no repository writes on failure.
...
- Manual evidence should use temporary committed and local-mode Git worktrees to demonstrate both trust classes while external, dirty/mixed, and symlinked inputs fail closed. Record commands, expected observations, actual observations, and cleanup.
