...
- A8. Inspect result contracts expose exactly the profile, root, aggregate digest, ordered sources, and nullable matching-receipt summary. Import result contracts expose exactly profile, root, aggregate digest, selected spec, draft, mapping, `applied`, receipt ID/path, and ordered predicted/resulting tasks. Additional internal claims cannot leak into either public shape.
